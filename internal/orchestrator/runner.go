...
type Config struct {
	...
	// BrowserFlowRunner enables separately granted, ephemeral accessible
	// click/wait flows. Nil keeps browser_flow out of every agent catalog.
...
func NewRunner(logger *slog.Logger, store taskstate.Store, tracer profiler.Tracer, cfg Config) *Runner {
	...
	agent := NewAgentLoopExecutor(nil, runner.shell, runner.file, runner.git, cfg.AgentLoopMaxModelCalls, agentLoopGatedTools(runner.policies), cfg.HTTPPolicy, WithWebSearchClient(cfg.WebSearch), WithBrowserInspector(cfg.BrowserInspector), WithBrowserFlowRunner(cfg.BrowserFlowRunner))
...
func (r *Runner) SetAgentLLMClient(llm AgentLLMClient) {
	agent := NewAgentLoopExecutor(llm, r.shell, r.file, r.git, r.config.AgentLoopMaxModelCalls, agentLoopGatedTools(r.policies), r.config.HTTPPolicy, WithWebSearchClient(r.config.WebSearch), WithBrowserInspector(r.config.BrowserInspector), WithBrowserFlowRunner(r.config.BrowserFlowRunner))
