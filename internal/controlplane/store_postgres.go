...
func NewPostgresStore(ctx context.Context, client *storage.PostgresClient, key string) (*PostgresStore, error) {
...
func (s *PostgresStore) Snapshot(ctx context.Context) (State, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *PostgresStore) UpsertAPIKey(ctx context.Context, key APIKey) (APIKey, error) {
...
func (s *PostgresStore) SetTenantEnabled(ctx context.Context, id string, enabled bool) (Tenant, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *PostgresStore) SetAPIKeyEnabled(ctx context.Context, id string, enabled bool) (APIKey, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *PostgresStore) RotateAPIKey(ctx context.Context, id, secret string) (APIKey, error) {
...
func (s *PostgresStore) migrate(ctx context.Context) error {
	...
	`, s.table))
	if err != nil {
		return fmt.Errorf("migrate postgres control plane store: %w", err)
	}
	return nil
}

func (s *PostgresStore) readState(ctx context.Context) (State, error) {
	var raw []byte
	err := s.db.QueryRowContext(ctx,
		fmt.Sprintf(`SELECT state FROM %s WHERE store_key = $1`, s.table),
		s.key,
	).Scan(&raw)
	...
	if err != nil {
		return State{}, fmt.Errorf("read control plane postgres state: %w", err)
	}
	if len(raw) == 0 {
		return State{}, nil
	}
	...
	if err := json.Unmarshal(raw, &state); err != nil {
		return State{}, fmt.Errorf("decode control plane postgres state: %w", err)
	}
	return cloneState(state), nil
}

func (s *PostgresStore) writeState(ctx context.Context, state State) error {
	payload, err := json.MarshalIndent(state, "", "  ")
	if err != nil {
		return fmt.Errorf("encode control plane postgres state: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("write control plane postgres state: %w", err)
	}
	return nil
}
