...
func NewFileStore(path string) (*FileStore, error) {
	if path == "" {
		return nil, fmt.Errorf("chat session file path is required")
	}
	store := &FileStore{
		path:     path,
...
func (s *FileStore) CreateSession(_ context.Context, session types.ChatSession) (types.ChatSession, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if session.ID == "" {
		return types.ChatSession{}, fmt.Errorf("session id is required")
...
func (s *FileStore) GetSession(_ context.Context, id string) (types.ChatSession, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	session, ok := s.sessions[id]
	if !ok {
...
func (s *FileStore) ListSessions(_ context.Context, filter Filter) ([]types.ChatSession, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	items := make([]types.ChatSession, 0, len(s.sessions))
	for _, session := range s.sessions {
...
func (s *FileStore) AppendTurn(_ context.Context, sessionID string, turn types.ChatSessionTurn) (types.ChatSession, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	session, ok := s.sessions[sessionID]
	if !ok {
...
func (s *FileStore) load() error {
	...
	if err != nil {
		...
		return fmt.Errorf("read chat sessions file: %w", err)
	}
	if len(payload) == 0 {
		return nil
	}
	...
	if err := json.Unmarshal(payload, &sessions); err != nil {
		return fmt.Errorf("decode chat sessions file: %w", err)
	}
...
func (s *FileStore) persistLocked() error {
	...
	if err != nil {
		return fmt.Errorf("encode chat sessions file: %w", err)
	}
	if err := os.MkdirAll(filepath.Dir(s.path), 0o755); err != nil {
		return fmt.Errorf("create chat sessions directory: %w", err)
	}
	if err := os.WriteFile(s.path, payload, 0o644); err != nil {
		return fmt.Errorf("write chat sessions file: %w", err)
	}
	return nil
}
