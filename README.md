...
> available, while broader workflow modes, browser typing and visual capture,
...
| **Browser tools**    | Optional local tools for native project-assignment Tasks. `browser_inspect` captures script-disabled static text; independent `browser_flow` runs one approved exact-origin sequence of 1–6 accessibility click/wait actions with scripts enabled. Every call uses a fresh process/profile and bounded text evidence. No typing, uploads, downloads, screenshots, reusable saved browser state, Hecate Chat, External Agents, or remote runtime. Private-IP controls apply per call. [See the contract.](docs/runtime/agent-runtime.md#browser-capabilities) |
...
- Browser typing, visual capture, retained sessions, and broader browser-backed
  QA beyond the narrow approved accessibility-flow slice.
...
4. Evaluate the built-in report-only `qa` workflow's manifest/report UX before
   ...
   engine.
5. Evaluate static browser evidence and the separately granted accessibility
   flow against their approval, privacy, and state-isolation boundaries before
   adding typing, screenshots, or retained sessions.
