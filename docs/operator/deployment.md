...
- **macOS Apple Silicon** is the maintainer-tested desktop path. Release bundles are required to be signed with a Developer ID Application certificate and notarized; the release workflow fails when its `APPLE_*` / `KEYCHAIN_PASSWORD` secrets are unavailable, then notarizes and verifies the exact DMG and verifies the exact updater payload/signature pair before upload (see [`macos-signing.md`](macos-signing.md)). Such bundles launch with no Gatekeeper warning and drag-install to `/Applications` cleanly. Earlier alpha bundles and explicit fork/validation artifacts can remain unsigned — Gatekeeper blocks the first launch with "Apple cannot check it for malicious software." Right-click `Hecate.app` → **Open** only when intentionally testing one of those artifacts; do not install it over the official app.
