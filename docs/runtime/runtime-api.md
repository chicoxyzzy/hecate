...
Approval responses may also include `action_summary`, an ordered, sanitized
description of the complete pending tool-call bundle covered by that decision.
This includes calls that would not independently require approval because
approving one gated call authorizes dispatch of the whole model response. The
summary is bounded to 16 lines, 512 UTF-8 bytes per line, and 4 KiB total.
`action_summary_incomplete=true` means at least one call or detail was omitted,
truncated, unrecognized, or deliberately withheld. Command bodies, file
contents, patches, HTTP headers and bodies, web queries, and MCP arguments are
not copied verbatim; the API exposes safe operational fields and byte/count
shapes instead. These fields are present on approval resources and SSE
snapshots, but are intentionally absent from public `approval.requested` and
`approval.resolved` event payloads so events do not become a second input-data
telemetry channel.
...
The Hecate Projects facade is backed by an embedded Cairnline SQLite store under Hecate's data directory. Cairnline owns portable coordination state; Hecate stores runtime defaults, execution references, chats, tasks, approvals, and traces separately. The public Hecate routes remain `/hecate/v1/projects*`, so the operator UI does not depend on Cairnline's transport. There is no selectable Hecate-native coordination backend or migration API. Chat sessions can carry an optional
...
Create and update responses are labelled with `read_backend: "cairnline"`
because Cairnline is the project-role authority.
...
Create and update responses are labelled with `read_backend: "cairnline"`
because Cairnline is the portable work-item authority.

#### `GET /hecate/v1/projects/{id}/work-items/{work_item_id}`

Returns one work item from embedded Cairnline or `404 not_found`, enriched with
Hecate assignment-runtime projection where execution references exist.
...
The endpoint first checks Hecate's assignment runtime overlay for a persisted
task/chat execution context packet whose refs match the requested project, work
item, and assignment. When that snapshot exists, it is returned so assignment
context matches the launched execution. Before start, the endpoint returns the
inspect-only assignment context assembled from embedded Cairnline. Older
executions that predate context snapshots may return `404 not_found`. The
Projects cockpit uses this endpoint for the assignment
...
project memory/context-source policies. For Hecate task assignments, it also
records the same `runtime` / `launch_readiness` metadata captured by preflight
so later run-context inspection can show what provider/model readiness looked
like when the assignment was started. The packet also includes the same
inspect-only `runtime` / `cairnline_launch_packet` evidence captured by
preflight. Hecate owns task creation, route checks, approvals, and execution;
Cairnline owns the coordinated assignment lifecycle. `include` /
`include_enabled` make the
...
Lists structured Cairnline handoff records for one work item, sorted by latest
...
prelude/context assembly reads project identity, role, skill, work, and
accepted-memory metadata from Cairnline. The chat transcript itself remains
Hecate chat state.
