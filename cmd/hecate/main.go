...
func main() {
	...
	if err := autoImportEnvProviders(context.Background(), logger, controlPlaneStore, cfg.Providers.OpenAICompatible); err != nil {
...
// autoImportEnvProviders mirrors each env-PRECONFIGURED entry into the
// control-plane store as a Provider row, so the admin UI shows a single
// source of truth instead of a hidden env-only branch. CP store edits
// win on conflict: if a row already exists for the same id we leave it
// alone and let the operator's admin-panel changes stand.
func autoImportEnvProviders(ctx context.Context, logger *slog.Logger, store controlplane.Store, configs []config.OpenAICompatibleProviderConfig) error {
	...
	importCtx := controlplane.WithActor(ctx, "system:env-import")
